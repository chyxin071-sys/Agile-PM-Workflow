...
description: "Guides novice PMs through a 5-step agile workflow: dialogue-based requirement gathering, drafting PRD skeleton, HTML prototype creation (via frontend-design), mermaid flowcharts, and final PRD with iframe slices."
---
...
新手往往难以一次性说清需求。你应首先采用「灵活对话式」引导，主动评估、温和追问、并进行总结确认。在需求未明确前，**不要急于创建文件或文件夹**。
...
### 3.1 产出内容
根据步骤一的确认信息，直接生成一份初步的 **HTML 格式**文档（存放在 `prd/` 目录下）。这份初版 PRD 必须搭建好完整的文档结构骨架（参考 6.1 节的标准目录），但在内容上只需重点填充前四部分：
1. 项目基本信息
2. 需求背景与目标
3. **用户使用场景（用户旅程图简述）**：用几句话描述典型用户的一天是如何使用该产品的。
4. 初步的功能清单与基础逻辑（不仅列出模块和名称，还需梳理出核心的操作主线和基本规则）。
*（后续章节如详细方案、流程图等可先留空，并注明“待原型确认后补充”）*
...
- 根据用户的反馈修改原型。
- **🚨 关键规则（PRD 实时同步）**：每次用户提出对原型的修改（如增加功能、修改交互逻辑、调整页面流转），你 **必须**在修改 HTML 原型的同时，同步更新第二版（详细版）PRD 中对应的“详细方案”表格。不仅要更新逻辑描述，还要确保切片引用路径与最新原型保持一致。
...
### 5.1 格式与结构要求
- 必须使用 **HTML 格式** 输出，包含完整的 CSS 样式和响应式布局，确保独立运行可用。
...
|---|---|---|---|
| (合并单元格) | 功能名称 | `<iframe src="../prototype.html#hash" style="width:375px; height:812px; transform:scale(0.7); transform-origin:0 0; border:none;" sandbox="allow-scripts allow-same-origin"></iframe>` | 1. 触发条件<br>2. 交互反馈<br>3. 异常处理 |
